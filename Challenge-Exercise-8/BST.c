...
Tree joinAVLTrees(Tree t1, Tree t2) {
	if (t1 == NULL)
    	return t2;
   	else if (t2 == NULL)
    	return t1;
		...
		while (left(curr) != NULL) {    // find min element in t2
      	...
      	left(curr) = t1;                // join version 1 for case 4
      	
		return t2;                    // right element is new root
   }
}
...
{
	// perform a standard BST removal
	if (t != NULL)
	{
		if (it < data(t))
			left(t) = TreeDelete(left(t), it);
		...
		else
		{
			...
			{
				// printf("t1: %d, t2: %d\n", data(left(t)), data(right(t)));
				new = joinAVLTrees(left(t), right(t));
				free(t);
				t = new;
			}
		}
		t = rebalance(t); // Rebalance
	}
...
/* Function to compute the maximum number of edges that can be removed to
   get a forest such that each connected component contains an even number of nodes.
*/
int maxEdgesToRemoveHelper(Tree root, int* numConnectedComponents) {
    if (root == NULL) {
        return 0;
    }

    int leftComponents = 0, rightComponents = 0;
    int leftEdges = maxEdgesToRemoveHelper(left(root), &leftComponents);
    int rightEdges = maxEdgesToRemoveHelper(right(root), &rightComponents);

    *numConnectedComponents = leftComponents + rightComponents;

    if ((leftComponents % 2 == 0) && (rightComponents % 2 == 0)) {
...
// Function to compute the maximum number of edges that can be removed
int maxEdgesToRemove(Tree root) {
    int numConnectedComponents;
    return maxEdgesToRemoveHelper(root, &numConnectedComponents);
}